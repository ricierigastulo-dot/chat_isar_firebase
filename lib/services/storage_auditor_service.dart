...
class StorageAuditorService {
  ...
  // ===========================================================================
  // ESTRATEGIA ANDROID: Escaneo seguro de directorios
  // ===========================================================================
    ...
    try {
      await for (final entidad in dir.list(recursive: true, followLinks: false).handleError((_) {})) {
        if (entidad is File) {
          ...
          if (['.jpg', '.jpeg', '.png', '.mp4', '.mkv', '.mov', '.3gp'].contains(ext)) {
            try {
              final stat = await entidad.stat();
              final bytes = await entidad.length();
              final tipo = ['.mp4', '.mkv', '.mov', '.3gp'].contains(ext) ? 'video' : 'foto';
  ...
  static Future<void> _escanearIOS(Isar isar) async {
    try {
      // Solicitar permisos nativos de iOS
      ...
      // Obtener álbumes de fotos y videos
  ...
  static Future<void> _guardarEnIsarSiNoExiste({
    ...
    if (existente == null) {
      final nuevoArchivo = MediaFileEntity()
        ...
        ..estadoSincronizacion = 0; // 0 = Pendiente por subir a Firebase
